...
echo [信息] 启动 Windows 系统信息查看工具 (GUI 版)... >> %LOGFILE%
echo.

REM 检查 EXE 是否存在
if not exist "SystemInfoViewer_GUI.exe" (
    echo [错误] 未找到 SystemInfoViewer_GUI.exe >> %LOGFILE%
    echo 错误：未找到 SystemInfoViewer_GUI.exe
    echo 请确保 EXE 文件在同一目录下
    goto :error
)

echo [信息] 找到 SystemInfoViewer_GUI.exe >> %LOGFILE%
echo [信息] 开始运行... >> %LOGFILE%
echo.

REM 运行 GUI EXE
SystemInfoViewer_GUI.exe
