...
echo [信息] 启动 Windows 系统信息查看工具 (台账专用版 v2.0)... >> %LOGFILE%
echo.

REM 检查 EXE 是否存在
if not exist "SystemInfoViewer_v2.exe" (
    echo [错误] 未找到 SystemInfoViewer_v2.exe >> %LOGFILE%
    echo 错误：未找到 SystemInfoViewer_v2.exe
    echo 请确保 EXE 文件在同一目录下
    goto :error
)

echo [信息] 找到 SystemInfoViewer_v2.exe >> %LOGFILE%
...
if %errorlevel% neq 0 (
    ...
    echo.
    echo 运行成功！
    echo 如需保存结果，请右键点击窗口 → 选择 → 复制
    echo 或者使用 - 台账版.bat 启动脚本
)
